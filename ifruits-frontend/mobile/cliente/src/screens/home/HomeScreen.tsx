  ...
  <TouchableOpacity
    ...
    onPress={() => {
      ...
      const fullStoreData = {
        ...
        name: item.name,
        logo: item.logo,
        banner: item.banner, // Banner padrão
        rating: item.rating,
        type: item.type,
...
export default function HomeScreen({ navigation }) {
  ...
  const [famousStores, setFamousStores] = useState<any[]>([]);
  ...
  useEffect(() => {
    const fetchStores = async () => {
      ...
      if (error) {
        console.log("Erro ao carregar lojas", error.message);
        return;
      }

      const storeFormatted = data.map((store, index) => ({
        id: store.id,
        name: store.nome,
        logo: require('../../assets/images/hortifruti-terra.png'),
        banner: require("../../assets/images/banner3.png"),
        rating: '4.8',
        type: store.categoria,
        distance: '2,5 km',
        time: '30 - 50 min',
      ...
      setStoreList(storeFormatted); 
      setFamousStores(storeFormatted);
      setStores(storeFormatted);
    }
